...
-- Libellé, fabricant.Coordonnées FROM `machine` JOIN fabricant ON machine.Id_fabricant = fabricant.n°Fabricant WHERE fabricant.Coordonnées LIKE "%Marseille%";
...
UPDATE machine
SET
    Libellé = 'Soudeuse RMX100'
WHERE
    Libellé = 'Soudeuse X100';
...
INSERT INTO operateur ("Nom", "Prénom") VALUES ("Huin", "Alexis")
...
FROM fabricant
WHERE
    fabricant.Coordonnées LIKE "%Lens%"
